...
"""
Direct test for Level 1 LangGraph implementation without imports
"""
...
class MockInputAgent:
    def process(self, input_data: str, metadata: Optional[Dict[str, Any]] = None):
        from collections import namedtuple
        Result = namedtuple('Result', ['content', 'modality', 'metadata'])
        return Result(
            content=input_data,
            modality="text" if ".txt" in input_data or input_data.startswith("This is") else "file",
            metadata=metadata or {}
        )

class MockModalityDetector:
    def detect(self, input_data: str) -> str:
...
class MockPreprocessor:
    def preprocess_file(self, file_path: str, modality: str):
        return f"[Processed {modality}: {file_path}]", {"status": "success"}
...
class Level1GraphAgentDirect:
    """Direct Level 1 Graph Agent for testing"""

    def __init__(self):
        """Initialize the Level 1 Graph Agent"""
        logger.info("Initializing Level 1 Graph Agent (Direct)")

        # Initialize mock agents
        self.input_agent = MockInputAgent()
        self.modality_detector = MockModalityDetector()
...
def test_level1_direct():
    """Test the direct Level 1 Graph Agent"""

    # Create the agent
    agent = Level1GraphAgentDirect()
