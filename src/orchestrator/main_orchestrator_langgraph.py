...
This module coordinates the entire workflow using LangGraph for Level 4 processing.
"""
...
class MainOrchestratorLangGraph:
    """Main orchestrator using LangGraph for Level 4 processing"""
    ...
    async def process_workflow(self, input_data: str, metadata: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Process the entire workflow from input to analysis using LangGraph for Level 4
        ...
        # Step 4: Level 4 Recommendations using LangGraph
        logger.debug("Level 4: Recommendations (LangGraph)")
        level4_result = level4_graph_orchestrator.process_recommendations(level3_result)
        logger.debug(f"Level 4 completed - Recommendation: {level4_result.get('summary', {}).get('recommendation', 'N/A')}")

        logger.debug("Workflow processing completed successfully with LangGraph")
