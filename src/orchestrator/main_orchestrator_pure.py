...
# Import pure LangGraph orchestrators
...
class MainOrchestratorPure:
    """Main orchestrator that uses pure LangGraph for all levels"""
    ...
    async def process_workflow(self, input_data: str, metadata: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Process the complete workflow using pure LangGraph for all levels
        ...
        logger.debug(f"Level 1 completed - Modality: {level1_result.get('modality', {}).get('modality', 'unknown')}")

        # Process Level 2 with pure LangGraph
        level2_result = level2_graph_orchestrator_pure.analyze_text(level1_result.get("preprocessing", {}).get("content", ""))
        logger.debug(f"Level 2 completed - Task Type: {level2_result.get('classification', {}).get('task_type', 'unknown')}")

        # Process Level 3 with pure LangGraph
        task_type = level2_result.get("classification", {}).get("task_type", "general")
        level3_result = level3_graph_orchestrator_pure.analyze_task(level1_result.get("preprocessing", {}).get("content", ""), task_type)
        ...
        logger.debug(f"Level 4 completed - Recommendation: {level4_result.get('enhanced_summary', {}).get('summary', {}).get('recommendation', 'N/A')}")
...
main_orchestrator_pure = MainOrchestratorPure()
