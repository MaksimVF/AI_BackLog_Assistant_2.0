...
#!/usr/bin/env python3
"""
Test script to verify enhanced duplicate detector functionality
"""
...
async def test_enhanced_duplicate_detector():
    """Test the enhanced duplicate detector with metadata tracking"""
    print("Testing Enhanced Duplicate Detector...")
    print("=" * 60)
    ...
    print(f"Original task ID: {result1['original_task_id']}")
    print(f"Duplicate attempts: {len(result1['duplicate_attempts'])}")
    ...
    async with AsyncSessionLocal() as db:
        # Delete any existing duplicate task first
        try:
            await TaskRepository.delete_task(db, "test_task_002")
        except:
            pass
    ...
    print(f"Duplicate attempts: {len(result4['duplicate_attempts'])}")
    print(f"Analysis: {result4['analysis']}")

    # Test 5: Verify metadata was updated
    print("\nTest 5: Verifying metadata was updated...")
    async with AsyncSessionLocal() as db:
        original_task = await TaskRepository.get_task_by_task_id(db, "test_task_001")
        if original_task and original_task.task_metadata:
            duplicate_attempts = original_task.task_metadata.get("duplicate_attempts", [])
            print(f"Original task metadata updated: {len(duplicate_attempts)} attempts recorded")
            for attempt in duplicate_attempts:
                print(f"  - Attempt at: {attempt.get('attempt_time')}")
        else:
            print("❌ Metadata not updated")

if __name__ == "__main__":
    asyncio.run(test_enhanced_duplicate_detector())
